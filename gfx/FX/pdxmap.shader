	...
	[[
		...
		float4 main( VS_OUTPUT_TERRAIN Input ) : PDX_COLOR
		{
		...
		#endif
			{
			    ...
			    //TestCam scales the camera height above sea level to the interval (0,8) with interest in making the transition start height = .5
				float testCam = ( ( vCamPos.y - WATER_HEIGHT - 50.0f ) * .004f );
				float colorRange = 0.7f;
				if ( testCam < 0.7f )
				{
				colorRange = testCam ;
				}
				colorRange = 0.7f * colorRange + 0.025f;
				
				//float2 vBlend = float2( 0.825f, 0.175f ); //left side adjusts brightness, right side transparency?, 0.65f, 0.15f || 0.5f, 0.4f || 0.9f, 0.25f
				float2 vBlend = float2( 0.825f - colorRange, colorRange ); //left side adjusts brightness, right side transparency?, 0.65f, 0.15f || 0.5f, 0.4f || 0.9f, 0.25f
