...
PixelShader = 
{
	...
	[[
		...
		float4 main( VS_OUTPUT_WATER Input ) : PDX_COLOR
		{
		...
		#ifdef PDX_OPENGLES
			float3 refractiveColor = float3(0.3, 0.5, 0.7);
			...
			float fresnelBias = 0.66f;
			float fresnel = saturate( dot( -vEyeDir, normal ) ) * 1.5f;
			fresnel = saturate( fresnelBias + ( 1.0f - fresnelBias ) * pow( 1.0f - fresnel, 1.0f ) );
			...
			refractiveColor = lerp( refractiveColor, waterColor.rgb, 1.0f+(1.0f*vIceFade) );
